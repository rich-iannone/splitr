...
#' @description This is a helper function that is used
#' by the 'hysplit_dispersion' function. Arguments
#' pertaining to the preset numbers for 'emissions',
#' 'grids', and 'species' in the 'hysplit_dispersion'
#' function are passed to the associated 'numbers'
#' argument in three instances of the
#' 'dispersion_preset_get' function. Each function
#' call creates a block of text that is used for
#' construction of a CONTROL file that is used in
#' conjunction with the HYSPLIT dispersion model
#' executable.
#' @param read the type of preset to get. The three
...
#' presets of a specified type (provided as the
#' 'read' character argument) to be retrieved. The
#' argument is to be provided as a single numeric
#' value or numeric vector. The numbers correspond
...
dispersion_preset_get <- function(read, numbers,
  ...
  # If duplicated values are found, stop the function
  # with a message
  if (no_duplicates == FALSE) {
    stop("The provided presets contain duplicate values.")
  }
  
  # Read in and extract values from the different
  # preset files that are available; these preset files
  # are: 'emissions', 'grids', and 'species'
  ...
  if (read == "emissions" & no_duplicates == TRUE) {
    ...
    if (length(numbers) == 1){
      ...
      in_range <- 
        ifelse(all(numbers %in% 1:number_of_entries),
               TRUE, FALSE)
      ...
      block <- 
        seq(from = ((numbers - 1) * 6) + 2,
            to = ((numbers - 1) * 6) + 5,
            by = 1)
      
      # Get vector of data elements for CONTROL script
      emissions <-
        c(1, as.vector(from_file[min(block):max(block),]))
    ...
    if (length(numbers) > 1){
      ...
      # Initialize 'emissions.list' list with
      # appropriate length
      list.from_file <-
        vector(mode = "list",
