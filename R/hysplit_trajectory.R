...
#' the meteorological data files relevant to the model options chosen. By
#' default, the location '~/Documents/SplitR/Met' is chosen.
#' @param path_output_files a full path should be provided for a location that
#' the trajectory output files will be written. By default, the location
...
#' from this location. By default, the location '~/Documents/SplitR/Working'
#' is chosen.
...
hysplit_trajectory <- function(traj_name = NULL,
  ...
  if (run_type == "day"){
    ...
    list_run_days <- 
      as.POSIXct(run_day, origin = "1970-01-01", tz = "UTC")
  } else if (run_type == "range"){
    
    list_run_days <- 
      seq(as.POSIXct(run_range[1], origin = "1970-01-01", tz = "UTC"),
          as.POSIXct(run_range[2], origin = "1970-01-01", tz = "UTC"),
          by = 86400)
  } else if (run_type == "years"){
    
    list_run_days <- 
      seq(as.POSIXct(paste0(substr(run_years, 1, 4),"-01-01"), 
  ...
  for (i in 1:length(list_run_days)){
    ...
    for (j in daily_hours_to_start){    
      ...
      end_time_GMT <-
        as.POSIXct(ifelse(backtrajectory == TRUE, 
