...
hysplit_dispersion_animation <- function(dispersion_df = NULL,
  ...
  # Stop function if no path for output files
  # is provided
  ...
  if (!is.null(dispersion_df)){
    ...
    if (is.integer(dispersion_df[,1]) &
        ...
        is.numeric(dispersion_df[,5])){
      valid_classes <- TRUE
    }
    
    if (valid_names == FALSE |
        valid_classes == FALSE){
  ...
  bbox_data <- 
    make_bbox(lon = dispersion_df$lon,
              lat = dispersion_df$lat)
  ...
  bounding_box <- function(lat, lon, dist, in.miles = TRUE){
    ...
    m <- 
      matrix(c(lon_range(lonr = lonr, dlon = dlon),
               lat_range(latr = latr, r = r)),
             nrow = 2, byrow = TRUE)
    
    dimnames(m) <- list(c("lng", "lat"),
                        c("min", "max"))
    m
  }
  
  # Determine the distance away from the center-point
  # to generate a bounding box for the map image that
  # encompasses the bounding box for the dispersion
  # data; this will keep generating new 'bbox_map'
  # objects until the map extents are greater than the
  # data extents
  for (i in seq(from = 0.2, to = 1000, by = 0.2)){
    
    bbox_map <- 
      bounding_box(lon = (bbox_data[[1]] +
                            bbox_data[[3]])/2,
                   lat = (bbox_data[[2]] +
                            bbox_data[[4]])/2,
                   i, in.miles = FALSE)
  ...
  # If chosen, a Stamen 'toner' style map that
  # encompasses the bounds of the dispersion data will
  # be downloaded
  if (map_type == "stamen"){
  ...
  # If chosen, an Open Street Maps 'terrain' style map
  # that encompasses the bounds of the dispersion data
  # will be downloaded
  if (map_type == "osm"){
    map <- get_map(location = bbox_map,
                   maptype = "terrain",
  ...
  # Build on the 'dispersion_df' data frame by
  # identifying each particle by hour of release;
  # first determine number of particles released from
  # source per hour
  particles_released_per_hour <- 
    nrow(subset(dispersion_df, hour == 1))
  
  # Determine the total number of hours
  total_hours <- max(dispersion_df$hour)
  
  # For each hour of particle positions in the
  # 'dispersion_df' data frame, classify
  # the particles by the hour of release
  for (i in 1:total_hours){
    
    if (i == 1){
      dispersion_df_hour_start <- 
    ...
    for (k in 1:i)
      particle_id <- 
        c(particle_id,
          seq(from = ((k - 1) * 
                        particles_released_per_hour) + 1,
              to = (k * particles_released_per_hour),
              by = 1))
    
    if (i == total_hours){
      
      # Add 'hour_start' and 'particle_id' vectors to
      # 'dispersion_df' data frame
      dispersion_df$hour_start <-
        dispersion_df_hour_start
      dispersion_df$particle_id <-
        particle_id
      
      # Remove vectors objects from memory
      rm(dispersion_df_hour_start,
         particle_id, i, j, k)
    }
  }
  
  # Begin loop to generate a large dataframe with
  # minutely particle positions
  for (i in 1:max(unique(dispersion_df$particle_id))){
    
    # Initialize a long data frame for single
    # particle movements for every particle
    if (i == 1){
      particle_df <-
        as.data.frame(mat.or.vec(nr = 0, nc = 6))
      colnames(particle_df) <-
        c("particle_id", "lon", "lat",
          "height", "hour", "hour_start")
    }
    
    # Generate a vector of evenly spaced time
    # increments for the duration of the particle
    # history
    vector_hour <- 
      seq(from = min(dispersion_df[dispersion_df$particle_no == i, ][,5]) - 1,
          to = max(dispersion_df[dispersion_df$particle_no == i, ][,5]) - (1/100),
          by = 1/100)
    
    # Use 'spline' function to generate latitude and
    # longitude particle positions for every
    # minute of travel from the source location
    vector_lon <- 
      spline(x = c(start_long_deg, dispersion_df[dispersion_df$particle_no == i, ][,2]),
    ...
    # Use the 'spline' function to generate particle
    # heights for every minute of travel from the
    # source location
    vector_hgt <- 
      spline(x = c(start_height_m_AGL, dispersion_df[dispersion_df$particle_no == i, ][,4]),
             n = length(dispersion_df[dispersion_df$particle_no == i, ][,2]) * 100)[[2]]
    ...
    vector_hour_start <- 
      rep(unique(dispersion_df[dispersion_df$particle_no == i, ][,6]),
          length(vector_lon))
    
    # Bind vectors into a data frame
    df_from_vectors <- 
      cbind(vector_particle_id, vector_lon,
            vector_lat, vector_hgt,
            vector_hour, vector_hour_start)
    colnames(df_from_vectors) <- names(particle_df)
    
    # Bind 'df_from_vectors' data frame into long
    # dataframe 'particle_df'
    particle_df <- rbind(particle_df, df_from_vectors)
  }
  
  # Remove objects from memory
  rm(df_from_vectors, vector_hour,
     vector_hour_start, vector_hgt,
     vector_lat, vector_lon, vector_particle_id,
     particles_released_per_hour, i)
  
  # Round the 'hour' column in 'particle_df' to
  # 2 decimal places
  particle_df$hour <- round(particle_df$hour, 2)
  
  # Round the 'height' column in 'particle_df' to
  # 2 decimal places
  particle_df$height <- round(particle_df$height, 2)
  
  # Write the 'particle_df' data frame to a CSV file
  # if it is requested
  ...
  # Obtain vector of unique, sorted, fractional hours
  # in 'particle_df'
  ...
  # Generate an image from particles at every
  # fractional hour
  for (i in 1:length(fractional_hours)){
    
    # Create a data frame that is a subset by
    # fractional hour
    particle_df_time <- 
      subset(particle_df, hour == fractional_hours[i])
    
    # Extract columns from 'particle_df_time' and
    # prepend the starting values
    smooth_x <- c(start_long_deg,
                  particle_df_time$lon)
    smooth_y <- c(start_lat_deg,
                  particle_df_time$lat)
    smooth_h <- c(start_height_m_AGL,
                  particle_df_time$height)
    
    # Create a data frame from the 3 vectors
    smooth_xyh <- 
      as.data.frame(cbind(smooth_x,
                          smooth_y,
                          smooth_h))
    ...
    # Generate a ggplot object from the
    # 'smooth_xyh' data frame
    gg <- ggmap(ggmap = map) +
    ...
    # Convert PDF file to a JPEG file using
    # ImageMagick, cropping whitespace
  ...
  # Construct a string with glob to pass into the
  # ffmpeg call
  dispersion_plot_glob <- 
    paste0("dispersion-map-", output_time, "-%06d.jpg")
  
  # Construct the movie name
  if (is.null(movie_output_name)){
    movie_output_name <- 
      paste0("dispersion_movie__", output_time)
  }
  
  # If a movie name is provided, ensure that the
  # filename won't be duplicated
  if (!is.null(movie_output_name)){
    
    if (paste0(movie_output_name, ".mov") %in%
        list.files(path = path_output_files)){ 
      movie_output_name <- 
        paste0(movie_output_name, "-", output_time)
    }
  }  
  ...
  if (.Platform$OS.type == "unix"){
    system(
      paste0("cd ", path_output_files,
             " ; ffmpeg -f image2 -start_number 1 -i '",
             dispersion_plot_glob, "' -r ",
             frame_rate, " ",
