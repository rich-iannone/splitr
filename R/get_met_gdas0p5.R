...
get_met_gdas0p5 <- function(files = NULL,
  ...
  download(
    url = paste0(gdas0p5_dir, "listing"),
  ...
  if (!is.null(files)) {
    ...
    if (files_available == TRUE) {
      ...
      for (i in 1:length(files)) {
        ...
        download(
          url = paste0(gdas0p5_dir, files[i]),
  ...
  if (!is.null(years)) {
    ...
    for (i in 1:length(the_list)) {
      ...
      download(
        url = paste0(gdas0p5_dir,the_list[i]),
  ...
  if (!is.null(months)) {
    ...
    if (all_files_available == FALSE) {
      ...
      for (i in 1:length(files_missing)) {
        ...
        dates[i] <- 
          as.character(ymd(gsub("^([0-9]*).*", "\\1", files_missing[i])))
      }
