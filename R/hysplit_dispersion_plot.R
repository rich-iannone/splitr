...
#' @description The function plots hourly outputs of
#' dispersed particles onto maps.
#' @param hours specification of hours of dispersion
#' data to plot.
...
hysplit_dispersion_plot <- function(hours = 'all',
  ...
  if (!is.null(dispersion_df)){
    ...
    valid_classes <- 
      ifelse(all(is.integer(dispersion_df[,1]) &
  ...
  # If value for 'hours' argument contains a vector
  # list, validate that vector to ensure those hours
  # are within the range of hours in the dispersion
  # dataframe
  if (is.vector(hours)){
    hours_dispersion_df <-
      unique(dispersion_df$hour)
    hours <-
      hours[which(hours %in% hours_dispersion_df)]
  }
  
  # Determine the extent of particle dispersion
  bbox_data <- make_bbox(lon = dispersion_df$lon,
                         lat = dispersion_df$lat)
  
  # Create 'bounding_box' function to provide a
  # square bounding box that's defined by the
  # center-point lat/lon, and the distance away in
  # kilometers
